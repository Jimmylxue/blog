...
如果转换的字符第一个是0，则会以八进制来转，如果是`ox`则会以十六进制来转，这就会触发一个更大的问题了：

```js
parseInt('08') === parseInt('09')  // true  相当于 0 === 0
/*
	8 和 9 在二进制中都不是合法数字，所以按照规则会直接跳过
*/
```

有时候可能是不小心写了这种比较，我们想以十进制来比较，想要表达的其实是`8 === 9`，但是却变成了八进制比较方法了，这要是出一个bug了相信找起来会花非常久的。

所以我们在使用`parseInt()`的时候，最好都加上第二个参数（基础参数）如：

```js
parseInt('08',10) === parseInt('09',10) // false
