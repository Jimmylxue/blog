...
1. 本地电脑生成 ssh 密钥：`ssh-keygen -t -rsa` 一路回车，会创建出对应的文件

2. 进入我们的 ssh 目录：`cd ~/.ssh` 这时候输入`ls` 会看到那几个密钥文件

3. 通过 `ssh-copy-id` 命令实现免密登录。具体这样执行：`ssh-copy-id -i ~/.ssh/id_rsa.pub 1.112.333.444` 即可。
